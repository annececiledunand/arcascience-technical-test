...
from src.eutils_retrieval.api import NCBIDatabase, NCBIEndpoint, call_eutils, call_eutils_async
...
async def search_and_store_async(query: str, db: NCBIDatabase) -> StorageInfos | None:
    """Search `db` for articles based on query and ask to store them for later retrieval.
    ...
    logger.debug(f"Calling {db.value} database for search and store.")
    search_data = await call_eutils_async(NCBIEndpoint.SEARCH, search_params)
    total_results = int(search_data["esearchresult"]["count"])
...
async def fetch_all_stored_articles_async(
    storage_infos: StorageInfos,
    max_allowed_elements: int = MAX_ALLOWED_SUMMARY_RETRIEVAL,
) -> dict | None:
    """Fetch all stored articles data requested in previous db query.
    ...
    for offset in range(0, total_elements, limit):
        ...
        if stored_summaries := await fetch_stored_articles_by_batch_async(
            storage_infos,
            offset=offset,
            limit=limit,
        ):
...
async def fetch_stored_articles_by_batch_async(
    storage_infos: StorageInfos,
    offset: int = 0,
    limit: int = MAX_ALLOWED_SUMMARY_RETRIEVAL,
) -> dict | None:
    ...
    summary_data = await call_eutils_async(NCBIEndpoint.SUMMARY, summary_params)
